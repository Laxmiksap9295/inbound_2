...
sap.ui.define([
	...
	return Controller.extend("ZGT_MM_INBOUND.controller.Truck_Reporting", {
		Formatter: Formatter,
		onInit: function () {
			// truck reporting is copy of gate entry by srinivas on 27/09/2025 
			oThat = this;
			oThat.Images = [];
			...
			this.oRouter.getRoute("TruckReporting").attachMatched(this._onRouteMatched, this);
		},
		//======================================================================================//
		...
		_onRouteMatched: function (oEvent) {
			...
			oThat.oView.byId("id_DeliveryF").setVisible(false);
			oThat.getView().byId("id_ParkingYard").setSelectedKey("");
									...
									MessageBox.show(oThat.oView.getModel("i18n").getResourceBundle().getText("Msg2"), {
										...
										onClose: function (oAction) {
											if (oAction === 'YES') {
												...
												// var sRead = "/DownloadSet(IvWbid='" + vWbid + "',IvPrint='X',TruckReport='X',IvBatch='" + vSelectedLots +  "')/$value";
					...
					if (vWtype == "TRIP") {
						...
						if (oThat.oView.byId("id_POBtn").getText() === oThat.oView.getModel("i18n").getResourceBundle().getText("Text3")) {
							...
							//end by Srinivas on 15/07/2025 for ASN					
								...
								oThat.PoStoFlag = oData.PoStoFlag // Added by srinivas on 31/10/2025 for intercompany
								if (oThat.oView.getModel("POST").getData().Wtype === "01" && oThat.PoStoFlag === "X") {
									oThat.oView.byId("id_DeliveryF").setVisible(true);
								} else {
									oThat.oView.byId("id_DeliveryF").setVisible(false);
								}
								oThat.oPoItemFrag = sap.ui.xmlfragment("ZGT_MM_INBOUND.Fragments.PoItemList", oThat);
		...
		onValueHelpConfirmPlant: function (oEvent) {
			...
			oThat.oView.getModel("POST").refresh(true);
			//added by srinivas on 02/10/2025 for truck reporting
			oThat.TruckReporting = oThat.SelectedPlant.TruckReport;
			if (oThat.TruckReporting === "X") {
				oThat.oView.byId("id_TruckSave").setEnabled(true);
				// parking yard dropdown 
				oThat.BusyDialog.open();
				var aFilters = [
					new Filter("Plant", sap.ui.model.FilterOperator.EQ, oSelectedItem.getDescription())
				];
				var oModel = oThat.getOwnerComponent().getModel(); // ODataModel
				oModel.read("/F4ParkingYardSet", {
					filters: aFilters,
					// urlParameters: {
					...
					success: function (oData) {
						...
						if (oData.results && oData.results.length > 0) {
							...
							oThat.oView.setModel(oJsonModel, "ParkingYardComboModel");
						}
				...
				//oThat.oView.getModel("POLIST").setData([]);
				var oPoListModel = oThat.oView.getModel("POLIST");
				if (oPoListModel) {
					oPoListModel.setData([]);
				}
				oThat.oView.byId("id_TruckSave").setEnabled(false);
				oThat.BusyDialog.close();
				MessageBox.error(oThat.oView.getModel("i18n").getResourceBundle().getText("TruckError"));
				return;
			}
			//ended by srinivas on 02/10/2025 for truck reporting
		...
		onCaptureCamera: function () {
			...
			if (!oThat.oCapture) {
				...
				oThat.oCapture = sap.ui.xmlfragment(oThat.getView().getId(), "ZGT_MM_INBOUND.Fragments.CaptureImage", oThat); // added by srinivas by commenting above code 
		...
		// 		var sMime = oItem.getMimeType();
		// 		var sFileName = oItem.getFileName();
		// 		if (!sUrl) {
		// 			sap.m.MessageToast.show("Document preview not available");
		// 			return;
		// 		}
		// 		var lowerMime = sMime ? sMime.toLowerCase() : "";
		// 		var lowerName = sFileName ? sFileName.toLowerCase() : "";
		...
		// 		// // Office files preview via Microsoft Office Online Viewer
		// 		// var officeExtensions = [".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx"];
		// 		// if (officeExtensions.some(ext => lowerName.endsWith(ext))) {
		// 		//     // var officeViewerUrl = "https://view.officeapps.live.com/op/view.aspx?src=" + encodeURIComponent(sUrl);
		// 		//     // window.open(officeViewerUrl, "_blank");
		// 		// // 	var googleViewerUrl = "https://docs.google.com/viewer?url=" + encodeURIComponent(sUrl) + "&embedded=true";
		// 		// // window.open(googleViewerUrl, "_blank");
		// 		// var link = document.createElement("a");
		// 		// link.href = sUrl;
		// 		// link.download = sFileName; // This ensures it keeps the same name
		...
		// 		// All other files → Download with same name
		// 		var link = document.createElement("a");
		// 		link.href = sUrl;
    ...
    // --- Remove any old image with same radio type ---
    ...
    // --- Get file object from UploadCollection ---
    var file = oEvent.getSource()._getFileUploader()._aXhr[0]?.file;
    if (!file) {
        sap.m.MessageToast.show("No file found for upload.");
        return;
    }
    ...
    if (file && file.name) {
        ...
        if (dotIndex !== -1) {
            fileExt = file.name.substring(dotIndex);
        }
    }
    ...
    // --- Clear old XHR reference (SAPUI5 internal cleanup) ---
    oEvent.getSource()._getFileUploader()._aXhr.splice(0, 1);

    // --- Show temporary feedback instead of full BusyDialog ---
    sap.m.MessageToast.show("Processing " + file.name + " ...", { duration: 1000 });

    // --- Process asynchronously to avoid UI blocking ---
    setTimeout(function () {
        console.time("FileCompress+Read");

        // 🧩 Compress the image (0.9 quality, 1280x720)
        oThat._compressImage(file, 0.9, 1280, 720).then(function (compressedFile) {
            ...
            reader.onloadstart = function () {
                sap.m.MessageToast.show("Encoding " + file.name + " ...", { duration: 800 });
            };
            ...
            reader.onloadend = function (evt) {
                ...
                oThat.BusyDialog.close();
                sap.m.MessageToast.show("File processed successfully", { duration: 1200 });
            };
            ...
            reader.onloadend = function (evt) {
                ...
                oThat.BusyDialog.close();
                sap.m.MessageToast.show("File added without compression", { duration: 1200 });
            };
            reader.readAsDataURL(file);
        });
    }, 0); // setTimeout yields to UI thread
},
				...
				if (flag == true) {
					...
					var oEntity = {
						"d": {
							...
							"IvWbid": IvWbid,
							"TruckReport": "X",
							"Process": oThat.getView().byId("id_ComboProcess").getSelectedItem().getText(), // srinivas
							"ParkingYard": oThat.getView().byId("id_ParkingYard").getSelectedItem()?.getText() || "",
							// "IvItem"		: IvItem,
							"PostReturnNav": [],
							"PostWbHeaderNav": [],
							"PostWbitemNav": [],
		...
		onScanDelivery: function (oEvent) {
			...
			var oBundle = oThat.getView().getModel("i18n").getResourceBundle();
			var oVideoDeviceModel = new JSONModel();
			// added by srinivas for Intercompany on 31/10/2025
			var oButton = oEvent.getSource();
			var sLocalId = this.getView().getLocalId(oButton.getId());
			var sTargetInputId = "";
			if (sLocalId === "id_ScanDeliveryIC") {
				sTargetInputId = "id_DeliveryICValue";
				oThat.oView.byId("id_Delivery").setValue("");
			} else if (sLocalId === "Id_ScanDelivery") {
				sTargetInputId = "id_Delivery";
				oThat.oView.byId("id_DeliveryICValue").setValue("");
			}
							...
							sap.ndc.BarcodeScanner.scan( //Desktop Version
								function (mResult) {
									if (!mResult.cancelled) {
										...
										oThat.getView().byId(sTargetInputId).setValue(mResult.text.trim()); // added by srinivas for Intercompany/ref delivery on 31/10/2025 
		...
		onSubmitDelivery: function () {
			...
			var oDICValue = oThat.oView.byId("id_DeliveryICValue").getValue();
			if (vValue != "") { // added only if  by srinivas on 31/10/2025 for Inter company
				if (oThat.oView.getModel("POLIST").getData().length == 0) {
					var oEntity = {
						"d": {
							"GateEntry": "",
			...
			//added by srinivas on 31/10/2025 for Inter company
			if (oDICValue) {
				oThat.BusyDialog.open();
				var aFilters = [new Filter("Delivery", sap.ui.model.FilterOperator.EQ, oDICValue)];
				var oModel = oThat.getOwnerComponent().getModel(); // ODataModel
				oModel.read("/GetDelHeaderSet", {
					filters: aFilters,
					success: function (oData) {
						oThat.BusyDialog.close();
						if (oData.results && oData.results.length > 0) {
							oThat.oView.byId("id_VehiNo").setValue(oData.results[0].VehicleNo || "");
							oThat.oView.byId("id_VehicleTy").setValue(oData.results[0].VehicleType || "");
							oThat.oView.byId("id_InDriver").setValue(oData.results[0].DriverName || "");
							oThat.oView.byId("id_InDriverMob").setValue(oData.results[0].MobileNo || "");
							oThat.oView.getModel("POST").getData().Challan = oDICValue;
						}
					}.bind(this),
					error: function (oError) {
						oThat.BusyDialog.close();
